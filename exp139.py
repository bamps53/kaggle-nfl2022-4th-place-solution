...
from models.v2 import TrackAnyTransformerYoloxHMGlobalFixedMask as Net
...
cfg.exp_name = 'exp139_hm_transformer_gc_more_aug_fixed_mask'
...
cfg.train.image_transforms = A.Compose([
    ...
    A.ShiftScaleRotate(p=1.0, shift_limit=0.2,
                       scale_limit=0.2, rotate_limit=10),
...
def inference_v2(cfg, model_dir, use_all_frames=False):
    ...
    for game_play in game_plays:
        ...
        os.makedirs(save_dir, exist_ok=True)
        for fold in range(4):
            fold_save_dir = f"{save_dir}_fold{fold}"
        ...
        for (game_play, p1, p2), group_df in tqdm(pred_df.groupby(['game_play', 'nfl_player_id_1', 'nfl_player_id_2'])):
            ...
            filled_features[not_null] = all_features[indices]
            for fold in range(4):
                file_name = f"{save_dir}_fold{fold}/{game_play}_p1_{p1}_p2_{p2}.npy"
                np.save(file_name, filled_features[:, :, fold])


def extract_feats_for_all_train_data(cfg, fold, model=None, test_dataloader=None, use_all_frames=None):
    if model is None:
        weight_path = f"{cfg.output_dir}/best_fold{fold}.pth"
        model = get_model(cfg, weight_path)
    model.eval()
    ...
    if test_dataloader is None:
        test_dataloader = get_val_dataframe_dataloader(cfg.valid, fold=-1, use_all_frames=False)
...
if __name__ == "__main__":
    ...
    for fold in range(args.start_fold, args.end_fold):
        ...
        if args.validate:
            ...
            extract_feats_for_all_train_data(cfg, fold)
            train_df = pd.read_csv(cfg.valid.label_df_path)
            merge(train_df, fold)
        else:
            train(cfg, fold)
