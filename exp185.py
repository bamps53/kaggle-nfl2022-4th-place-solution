...
from models.unet1d import ImageFeatUNet1dGroundGlobalLN as Net
...
cfg.exp_name = 'exp185_stage2_from_exp139_ground_all_feats_v3_additional_LN'
...
cfg.train.ground_only = True
cfg.train.pair_cols = col_info['pair']
cfg.train.player1_cols = col_info['p1'] + col_info['p1_additional']
...
cfg.valid.ground_only = True
cfg.valid.pair_cols = col_info['pair']
cfg.valid.player1_cols = col_info['p1'] + col_info['p1_additional']
...
cfg.test.ground_only = True
cfg.test.image_feature_dir = './test_preprocessed_data'
cfg.test.pair_cols = col_info['pair']
cfg.test.player1_cols = col_info['p1'] + col_info['p1_additional']
...
cfg.model.in_player_channels = len(col_info['p1'] + col_info['p1_additional'])
...
def train(cfg, fold):
    ...
    for epoch in range(init_epoch, cfg.epochs):
        ...
        for j, itr in enumerate(progress_bar):
            ...
            labels = inputs['ground'][masks].cpu().numpy()
            preds = outputs['ground'][masks].sigmoid().detach().cpu().numpy()

            all_losses.append(loss_dict['ground'].item())
...
def validate(cfg, fold, model=None, test_dataloader=None):
    ...
    for i, inputs in enumerate(tqdm(test_dataloader)):
        ...
        batch_labels = inputs['ground'].cpu().numpy()
        batch_preds = outputs['ground'].sigmoid().detach().cpu().numpy()
...
def inference(cfg):
    ...
    df = pd.concat(dfs).reset_index(drop=True)
    df.to_csv(f'./data/ground_preds_df.csv', index=False)
